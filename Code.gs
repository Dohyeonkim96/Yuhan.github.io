/****************************************************************
 * 통합 관리 시스템 v2.6 - 서버 스크립트 (Code.gs)
 * @description
 * - SheetService 헬퍼 객체로 데이터 조회 로직을 중앙화하고 코드 중복을 제거.
 * - 각 함수의 오류 처리 및 로깅을 강화하여 안정성 증대.
 * - 사용자 요청에 따른 커스텀 정렬 및 데이터 가공 로직 추가.
 ****************************************************************/

const CONFIG = {
  SHIPPING_SUMMARY: {
    SSID: '1v1XBTKxtBbhUC8lfAbGZwDGZCPSIPVML__2BNZKlBd0',
    SHEET_NAME: '시트1', HEADER_ROW: 2, DATA_START_ROW: 3
  },
  ORDER_MGMT: {
    SSID: '1b3Bn0JFjhhlcs0SPPkNFH7SdhMNjO_-zKOxSysn9EVI',
    DB_SHEET: '기본정보', ORDER_SHEET: '발주현황',
    PRODUCTION_PLAN_SHEET: "생산계획", INVENTORY_SHEET: "재고조회",
    SHIPPING_REQUEST_SHEET: "출고요청", DELIVERY_HISTORY_SHEET: "납품이력(25년)"
  }
};
...
function getOrderStatusDashboardData(filters = {}) {
  try {
    const dbData = SheetService.getDataAsObjects({ ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.DB_SHEET, headerRow: 2, dataStartRow: 3 });
    ...
    const orderData = SheetService.getDataAsObjects({ ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.ORDER_SHEET, headerRow: 1, dataStartRow: 2 });
    ...
    const historyData = SheetService.getDataAsObjects({ ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.DELIVERY_HISTORY_SHEET, headerRow: 1, dataStartRow: 2 });
...
function getInventoryDetails() {
  try {
    ...
    const invData = SheetService.getDataAsObjects({ ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.INVENTORY_SHEET, headerRow: 1, dataStartRow: 2 });
...
function saveReceivedStock(receivedItems) {
  try {
    const sheet = SheetService._getSheet(CONFIG.ORDER_MGMT.SSID, CONFIG.ORDER_MGMT.INVENTORY_SHEET);
...
function submitOrderToSheet(orders) {
  try {
    const sheet = SheetService._getSheet(CONFIG.ORDER_MGMT.SSID, CONFIG.ORDER_MGMT.ORDER_SHEET);
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].map(h => String(h).trim());
    const requiredHeaders = ['발주일', '유한품번', '지에스켐 품번', '품명', '수량', '납기일', '사업부'];
    for (const rh of requiredHeaders) {
        if (!headers.includes(rh)) return { success: false, message: `'${CONFIG.ORDER_MGMT.ORDER_SHEET}' 시트에 필수 헤더 '${rh}'가 없습니다.` };
    }
...
function saveProductionPlanItems(planDataArray) {
  try {
    const ss = SheetService._getSpreadsheet(CONFIG.ORDER_MGMT.SSID);
...
function getProductionPlansForCalendar(year, month) {
  try {
    const data = SheetService.getDataAsObjects({ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.PRODUCTION_PLAN_SHEET, headerRow: 1, dataStartRow: 2});
...
function deleteProductionPlan(sheetRowIndex) {
  try {
    ...
    const sheet = SheetService._getSheet(CONFIG.ORDER_MGMT.SSID, CONFIG.ORDER_MGMT.PRODUCTION_PLAN_SHEET);
    if (sheetRowIndex > sheet.getLastRow()) return { success: false, message: `삭제할 행(${sheetRowIndex})이 시트 범위를 벗어났습니다.` };
    sheet.deleteRow(sheetRowIndex);
...
function getPendingShipments() {
  try {
    const sheet = SheetService._getSheet(CONFIG.ORDER_MGMT.SSID, CONFIG.ORDER_MGMT.SHIPPING_REQUEST_SHEET);
    if (sheet.getLastRow() < 2) return [];
    const data = SheetService.getDataAsObjects({ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.SHIPPING_REQUEST_SHEET, headerRow: 1, dataStartRow: 2});
...
function updateShipmentRequest(requestData) {
  try {
    ...
    const shippingSheet = SheetService._getSheet(CONFIG.ORDER_MGMT.SSID, CONFIG.ORDER_MGMT.SHIPPING_REQUEST_SHEET);
    ...
    if (colMap.pallet > 0) shippingSheet.getRange(rowId, colMap.pallet).setValue(itemsPerPallet > 0 ? Math.ceil(newQuantity / itemsPerPallet) : 0);
...
function confirmShipment(rowId) {
  ...
  try {
    ...
    const historyLog = historyHeaders.map(header => historyLogObject[header] || '');
...
function getAvailableHistoryYears() {
  try {
    const sheet = SheetService._getSheet(CONFIG.ORDER_MGMT.SSID, CONFIG.ORDER_MGMT.DELIVERY_HISTORY_SHEET);
  ...
  } catch (e) { Logger.log(`getAvailableHistoryYears Error: ${e.stack}`); return [new Date().getFullYear()]; }
}

function getDeliveryHistory(filters) {
  try {
    const data = SheetService.getDataAsObjects({ ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.DELIVERY_HISTORY_SHEET, headerRow: 1, dataStartRow: 2 });
    const filteredData = data.filter(row => {
      const rowDate = parseDateString(row['납품일자']);
...
function getPoStatusDetails(filters = {}) {
  try {
    const dbData = SheetService.getDataAsObjects({
      ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.DB_SHEET, headerRow: 2, dataStartRow: 3
    });
    const itemMaster = new Map();
    const categorySet = new Set();
    const businessUnitSet = new Set();
    dbData.forEach(row => {
      ...
      if (code) {
        ...
        const businessUnit = String(row['사업부'] || '미분류').trim();
        itemMaster.set(code, {
          itemName: String(row['품명']).trim(),
          gskemPN: String(row['지에스켐 품번']).trim(),
          category: category,
    ...
    const orderData = SheetService.getDataAsObjects({
        ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.ORDER_SHEET, headerRow: 1, dataStartRow: 2
    });
    const orders = new Map();
    orderData.forEach(row => {
      const code = String(row['유한품번'] || '').trim();
    ...
    const historyData = SheetService.getDataAsObjects({
      ssid: CONFIG.ORDER_MGMT.SSID, sheetName: CONFIG.ORDER_MGMT.DELIVERY_HISTORY_SHEET, headerRow: 1, dataStartRow: 2
    });
    const deliveries = new Map();
    historyData.forEach(row => {
        const code = String(row['품목코드'] || '').trim();
